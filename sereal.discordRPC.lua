...
script_name = "Aegisub - Discord Rich Presence"
script_description = "Menghubungkan progress Aegisub ke Discord Rich Presence"
script_author = "Serenium"
script_version = "1.00"
...
function update_rpc()
    if (aegisub.project_properties() ~= nil) then
        ...
        if (videoname ~= " ") then
            ...
            aegisub.debug.out("Pastikan berkas Anda memiliki jalur fail video yang ditentukan sebelum memperbarui RPC.")
        end
    else
        aegisub.debug.out("Pastikan berkas Anda memiliki jalur fail video yang ditentukan sebelum memperbarui RPC.")
    end
...
aegisub.register_macro("Aegisub - RPC", "Hubungkan ke Discord Rich Presence", update_rpc)
